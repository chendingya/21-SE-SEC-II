...
const paymentStrategy = (id: number, status: string) => {
  request({
    url: `/order/${id}`,
    method: 'PATCH',
    data: {
...
const refund = (id: number) => {
  ElNotification({
    offset: 70,
    title: '真的要退款吗',
    message: h('success', { style: 'color: teal' }, '真的吗真的吗不再看看嘛'),
  })
  refundok(id);
}
const refundok = (id: number) => {
  request({
    url: `/order/${id}`,
    method: 'PATCH',
    data: {
  ...
  <div style="display: flex; flex-direction: column">
    ...
    <div style="margin-top: 5vh">
      ...
      <el-descriptions :column="4" border>
        <el-descriptions-item label="实付金额:" :span="2" width="25%" align="center" v-if="orderDetail.data">
          {{ orderDetail.data.real }}
        </el-descriptions-item>
        <el-descriptions-item label="积分变化:" :span="2" width="25%" align="center" v-if="orderDetail.data">
          {{ orderDetail.data.change }}
        </el-descriptions-item>
      </el-descriptions>
    ...
    <div style="margin-top: 2vh" v-if="orderDetail.data && orderDetail.data.status === '等待支付'">
      <div style="float:right;">
        ...
        <el-button type="primary" @click="paymentStrategy(id ?? -1, '支付宝支付')">
          支付宝支付
        </el-button>
        <el-button type="primary" @click="paymentStrategy(id ?? -1, '微信支付')">
          微信支付
        </el-button>
    ...
    <div v-else-if="orderDetail.data && orderDetail.data.status === '已完成'" style="margin-top: 2vh">
      <div style="float:right;">
        <el-button type="danger" @click="refund(id ?? -1)">
          退款
        </el-button>
      </div>
    </div>
