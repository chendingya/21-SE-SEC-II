...
const setForm = async () => {
  ...
  form.money = user.money;
}
const setChargeUser = async () => {
  chargeuser.username = user.username;
}
...
const charge = () => {
  ...
  request({
    url: `/user/${chargeuser.username}/${chargeuser.money}`,
    method: 'PATCH',
    data: {
      name: chargeuser.username,
      money: chargeuser.money,
      status: chargeuser.payment
    }
  }).then((res) => {
    ElNotification({
      offset: 70,
  ...
  <div style="display: flex; flex-direction: column">
    ...
    <el-form ref="ruleFormRef" :model="form" :rules="rules" label-width="30%" class="demo-ruleForm" label-position="right"
      ...
      <el-form-item label="积分" prop="points">
        <el-input v-model="form.points" style="width: 25vh" :disabled="!edit" />
      </el-form-item>
      <el-form-item label="余额" prop="money">
        <el-input v-model="form.money" style="width: 25vh" :disabled="!edit" />
      </el-form-item>

      <el-col :span="2" :offset="21">
        <el-button style="float:right" type="primary" @click="dialog = true;">
          充值
        </el-button>
      </el-col>

      <el-dialog destroy-on-close v-model="dialog" title="充值" width="30%">
        <el-form-item label="充值金额" prop="money">
          <el-input v-model="chargeuser.money" style="width: 30vh"/>
        </el-form-item>
        <div v-if="!emitCharge" style="display: flex; flex-direction: row-reverse; align-items: flex-end">
          <el-select v-model="chargeuser.payment" placeholder=" " style="width: 25vh">
            <el-option value="支付宝支付" />
            <el-option value="微信支付" />
          </el-select>
          <el-button type="primary" @click="emitCharge = true; charge()">充值</el-button>
        </div>
      </el-dialog>
    </el-form>
  </div>
